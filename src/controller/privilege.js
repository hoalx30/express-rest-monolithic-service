...
const findAll = async (req, res, next) => {
	try {
		...
		const { response: queried, pageable: paging } = await privilegeService.findAll({ page: +page, size: +size, nextCursor: +nextCursor });
...
const findAllBy = async (req, res, next) => {
	try {
		...
		const { response: queried, pageable: paging } = await privilegeService.findAllBy({ page: +page, size: +size, cond, nextCursor: +nextCursor });
