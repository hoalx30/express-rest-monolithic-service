...
const findAll = async (req, res, next) => {
	try {
		...
		const { response: queried, paging } = await userService.findAll({ page: +page, size: +size, nextCursor: +nextCursor });
...
const findAllBy = async (req, res, next) => {
	try {
		let { page, size, username, nextCursor } = req.query;
		if (page >= +SEEK_PAGING_BOUNDARY) nextCursor = page * size + 1;
		const cond = _.pickBy({ username }, (v) => !_.isNil(v));
		const { response: queried, paging } = await userService.findAllBy({ page: +page, size: +size, cond, nextCursor: +nextCursor });
